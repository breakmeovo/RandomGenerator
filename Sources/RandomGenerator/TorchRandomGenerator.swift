...
public class TorchRandomGenerator: RandomGenerator {
    ...
    public func nextNormal(mean: Double = 0.0, stdev: Double = 1.0) -> Double {
    ...
    public func nextArray(shape: [Int], mean: Double, stdev: Double) -> MLShapedArray<Double> {
    ...
    func normalArray(count: Int, mean: Double = 0.0, stdev: Double = 1.0) -> [Double] {
    ...
    /// This simulates torch.randn([1, 4, 64, 64], dtype=torch.float), note that for dtype=torch.double, it
    /// will be slightly different.
    func normalArray(count: Int, mean: [Double], stdev: [Double]) -> [Double] {
        // If it is smaller than 16 elements, Torch generates from Box-Muller transform directly.
        // Note that even if this is used to generate Float, it will use Double underneath.
        guard count >= 16 else {
            return (0..<count).map { i in nextNormal(mean: mean[i], stdev: stdev[i]) }
        }
        // Otherwise, Torch first fill a uniform distribution array, then do Box-Muller
        // transformation over this array.
        var data = (0..<count).map { _ in Double(nextFloat()) }
        for i in stride(from: 0, to: count - 15, by: 16) {
            for j in 0..<8 {
                ...
                let radius = sqrt(-2.0 * log(u1))
                let theta = 2.0 * .pi * u2
                data[i + j] = radius * cos(theta) * stdev[i + j] + mean[i + j]
                data[i + j + 8] = radius * sin(theta) * stdev[i + j + 8] + mean[i + j + 8]
            }
        }
        if count % 16 != 0 {
            for i in (count - 16)..<count {
